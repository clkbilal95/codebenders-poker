...
export default function RoomPage() {
  ...
  useEffect(() => {
    ...
    const handleLeave = () => {
      supabase.from('players').update({ online: false }).eq('id', playerId)
    }
    ...
    const roomSub = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'rooms', filter: `id=eq.${roomId}` }, payload => {
        if (payload.new) {
          ...
          if (payload.new.votes_visible === false && payload.old?.votes_visible === true) {
            setMyVote(null)
          }
        }
      })
    ...
    const giftSub = supabase
      ...
      .on('broadcast', { event: 'gift' }, ({ payload }) => {
        if (payload.toId === playerId) {
          ...
          if (payload.fromId !== playerId) {
            addToast(`${fromName} → ${toName}: ${payload.giftEmoji}`, payload.giftEmoji, 'gift')
          }
        }
      })
      .subscribe()
  ...
  async function revealVotes() {
    await supabase
      .from('rooms')
      .update({ votes_visible: true })
      .eq('id', roomId)
    addToast('Oylar açıldı!', '🎴', 'reveal')
  }
        ...
        <div className="relative z-10 min-h-screen flex flex-col items-center justify-center px-4">
          <div className="w-full max-w-md">
            <div className="text-center mb-8">
              ...
              <h1 className="text-3xl font-bold" style={{ fontFamily: 'Space Grotesk', color: '#6C63FF' }}>
                Odaya Katıl
              </h1>
              <p className="text-sm mt-1" style={{ color: '#7B82A8' }}>
                Oda: <span style={{ color: '#8B85FF', letterSpacing: 2 }}>{roomId}</span>
              </p>
            </div>

            <div className="rounded-2xl p-6 border" style={{ background: '#151929', borderColor: '#2A3050' }}>
              {/* Avatar picker */}
              <p className="text-sm font-medium mb-3" style={{ fontFamily: 'Space Grotesk', color: '#E8EAFF' }}>
                Avatarını seç
              </p>
              <div className="grid grid-cols-8 gap-2 mb-5">
                {AVATARS.map(av => (
                  <button
                    ...
                    style={{
                      background: selectedAvatar === av ? 'linear-gradient(135deg, #6C63FF, #8B85FF)' : '#1E2438',
                      border: selectedAvatar === av ? '2px solid #8B85FF' : '2px solid #2A3050',
                      cursor: 'pointer',
              ...
              {/* Name input */}
              <p className="text-sm font-medium mb-2" style={{ fontFamily: 'Space Grotesk', color: '#E8EAFF' }}>
                Adın
              </p>
              <input
                type="text"
              ...
              {/* Preview */}
              {playerName && (
                <div className="flex items-center gap-3 p-3 rounded-xl mb-4" style={{ background: '#0D0F1A' }}>
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl"
                    style={{ background: 'linear-gradient(135deg, #6C63FF33, #6C63FF55)', border: '2px solid #6C63FF' }}>
                    {selectedAvatar}
                  </div>
                  <div>
                    <p className="text-sm font-semibold" style={{ fontFamily: 'Space Grotesk', color: '#E8EAFF' }}>{playerName}</p>
                    <p className="text-xs" style={{ color: '#7B82A8' }}>Codebenders üyesi</p>
                  </div>
                </div>
              )}
              ...
              <button
                ...
                style={{
                  background: playerName.trim() ? 'linear-gradient(135deg, #6C63FF, #8B85FF)' : '#2A3050',
                  fontFamily: 'Space Grotesk',
                  cursor: playerName.trim() ? 'pointer' : 'not-allowed',
                  color: playerName.trim() ? 'white' : '#7B82A8',
                }}
              >
                🚀 Oyuna Gir
              </button>
    ...
    <>
      ...
      <div className="relative z-10 min-h-screen flex flex-col">
        ...
        <header
          ...
          style={{ borderColor: '#2A3050', background: 'rgba(13, 15, 26, 0.8)', backdropFilter: 'blur(10px)' }}
        >
          <div className="flex items-center gap-3">
            <span className="text-2xl">🃏</span>
            <div>
              <h1 className="font-bold text-sm" style={{ fontFamily: 'Space Grotesk', color: '#6C63FF' }}>
                Codebenders Poker
              </h1>
              <p className="text-xs" style={{ color: '#7B82A8' }}>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              style={{ background: '#1E2438', border: '1px solid #2A3050', color: '#7B82A8', cursor: 'pointer', fontFamily: 'Space Grotesk' }}
            >
              🔗 Link kopyala
            </button>
            <div
              className="text-xs px-3 py-2 rounded-lg"
              style={{ background: '#1E2438', border: '1px solid #2A3050', color: '#7B82A8' }}
            >
              👥 {players.length}
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col lg:flex-row gap-0">
          {/* Main game area */}
          <main className="flex-1 px-4 py-6 flex flex-col gap-6" style={{ maxWidth: 720, margin: '0 auto', width: '100%' }}>

            {/* Story input */}
            <div className="rounded-2xl p-4 border" style={{ background: '#151929', borderColor: '#2A3050' }}>
              <div className="flex items-center gap-2 mb-2">
                <span>📝</span>
                <span className="text-sm font-medium" style={{ fontFamily: 'Space Grotesk', color: '#E8EAFF' }}>
                  Oylanacak konu
                </span>
              </div>
              <input
                type="text"
                placeholder="Hikaye / ticket başlığını buraya yaz..."
                value={story}
            ...
            {/* Players row */}
            <div>
              <p className="text-xs mb-3" style={{ color: '#7B82A8', fontFamily: 'Space Grotesk' }}>
                ...
                {players.map(player => (
                  ...
                  >
                    <PlayerCard
                      ...
                      myRef={player.id === playerId ? myCardRef : null}
                    />
                  </div>
                ))}
                {players.length === 0 && (
                  <p className="text-sm" style={{ color: '#3D4466' }}>Henüz kimse yok...</p>
                )}
              </div>
            </div>
              ...
              {!votesVisible && (
                <>
                  <div
                    ...
                    style={{
                      background: allVoted ? 'rgba(61, 255, 160, 0.1)' : '#1E2438',
                      border: `1px solid ${allVoted ? 'rgba(61, 255, 160, 0.3)' : '#2A3050'}`,
                      color: allVoted ? '#3DFFA0' : '#7B82A8',
                      fontFamily: 'Space Grotesk',
                    }}
                  >
                    {allVoted ? '✅ Herkes oyladı!' : `⏳ ${players.filter(p => p.vote).length}/${players.length} oy`}
                  </div>
                  {allVoted && (
                    <button
                      onClick={revealVotes}
                      className="px-5 py-2 rounded-xl font-semibold text-white text-sm transition-all hover:scale-105 animate-pulse-glow"
                      style={{
            ...
            {/* Vote results */}
            {votesVisible && <VoteResults players={players} />}
